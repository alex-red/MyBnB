...
.modal#commentModal
  .modal-content
    h5 Commenting on 
      span: em {{context.context}}
    a.btn-floating.close-btn(@click="closeModal"): i.material-icons close
    form.col.s12
      .row
        .input-field.col.s12
          textarea#comment_text.materialize-textarea(length="500" v-model="inputs.comment_text")
          label(for="comment_text")

    span.error-text(v-show="inputs.error") Could not send your comment!
  .modal-footer
    button.btn(@click="submitComment" type="submit").color-primary submit
